...
let rec transl_conclusion_query next_f cur_state = function
  ...
  | QFalse -> ()
  | QEvent(QSEvent(inj,ord_fun,occ,at,t)) ->
      assert(inj = None && ord_fun = [] && occ = None && at = None);
      TermsEq.close_term_eq (fun t1 ->
  ...
  | QEvent(QGr _) -> Parsing_helper.internal_error "[lemma.ml >> transl_premise_event] Lemma should not contain strict inequalities."
  | QEvent(QIsNat t) ->
...
let continue_searching ev state = match ev with
  | QFact({ p_info = [Mess(n,_)]; _},_,_,_) when state.current_phase < n -> false
  ...
  | Before(evl,concl_q), ext ->
      ...
      try
        if Reduction_helper.has_name_q (Before(evl,concl_q))
          || List.exists (function QFact({ p_info = [Subterm _]; _},_,_,_) -> true | _ -> false) evl
        then
          begin
      ...
      with AxiomNotVerified ->
        if is_axiom
        then Parsing_helper.user_error "False axiom."
        else false

let check_axioms final_state axioms =
  List.for_all (fun (rq,is_axiom) -> check_one_axiom final_state is_axiom rq) axioms

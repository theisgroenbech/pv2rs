...
let catch_fail t =
  if Terms.term_never_fail t then
    ...
    let catch_fail = Terms.glet_fun (Terms.get_term_type t) in
    FunApp(catch_fail, [t])
	  ...
	  proc_layer_cond cond_eval (fun c ->
		...
		begin
		  if Terms.equal_terms c Terms.true_term then
		    ...
		    Terms.make_basic_let b (add_cond_eval cond_eval (catch_fail c))
		      (proc_layer_then (Var b) (fun tthen ->
			proc_layer_else (FunApp(Terms.is_false(), [Var b])) (fun telse ->
			  let gtest_symb = Terms.gtest_fun term_then.et_type in
			  proc_context (FunApp(gtest_symb, [FunApp(undo_catch_fail, [Var b]); tthen; telse])))))
		end
	      else
		let b = Terms.new_var_def Param.bool_type in
		Terms.make_basic_let b (add_cond_eval cond_eval (catch_fail c))
	  ...
	  | _ ->
		...
		proc_layer_term cond_eval (fun term_eval ->
		  if Terms.is_failure term_eval then
		    ...
		    Terms.make_basic_let v (add_cond_eval cond_eval (catch_fail term_eval))
		      (proc_layer_pattern (fun bound test ->
			let test_v = Terms.new_var_def Param.bool_type in
			Terms.make_basic_let test_v (Terms.make_and (FunApp(Terms.not_caught_fail_fun term.et_type, [Var v])) test)

...
let rec translateTerm t  =
  ...
  | FunApp(f,l) ->
      if (String.length (fixed_or_renamable f.f_name)) = 0 then
        ...
        VVar((fixed_or_renamable f.f_name))
      else
        VFun((fixed_or_renamable f.f_name), List.map translateTerm l)
...
let rec freeVars pat =
  match pat with
    PatVar x -> [x.vname.orig_name]
  | PatTuple (f, plist) -> List.concat (List.map freeVars plist)
  | PatEqual t ->
      match t with
        Var b -> [b.vname.orig_name]
      | FunApp(f,l) -> [(fixed_or_renamable f.f_name)]
...
let rec translate t env =
  ...
  | Restr (f, (args,_), p, occ) ->
      VExists([(fixed_or_renamable f.f_name)], translate p env)
  | Test (t, p, q,occ) ->
...
let translateEquation eq ttl =
  ...
  VExists([(fixed_or_renamable eq.f_name)],VPred("const",[]))
end
else begin
  ...
  VPling(VForall(params, VLolli(createTensors preds, VPling(VPred("k",[VFun((fixed_or_renamable eq.f_name),vars)])))))
end
...
let translateFuncs f =
  match f.f_cat with
    Red (r::rs) -> translateRewriteRule ((fixed_or_renamable f.f_name)) r (* ONLY RUNS ON FIRST ELEMENT; FIX*)
...
let parse state =
  match state.pi_process_query with
    ...
    let frees = (List.map(fun f -> f.f_name) (List.rev state.pi_freenames)) in
    let strings = List.map (fun s -> (fixed_or_renamable s)) frees in
    let env = frees in
    let f = VExists(strings, VTensor((createTensors (List.map(fun f -> VPling(VPred("k", [VVar(f)]))) (strings))), translate (p.proc) strings)) in
